...
class ImportCsv extends Component
{
    ...
    private function loadPreview(): void
    {
        ...
        // Group rows by nomor_form for preview (show first 3 forms)
        $allRows = [];
        ...
        // Preview first 3 forms (all their rows)
    ...
    public function processData(): void
    {
        ...
        // Read all rows and group by nomor_form
        $allRows = [];
        $rowNumber = 1;
        while (($row = fgetcsv($handle)) !== false) {
            $rowNumber++;
        ...
        // Group by nomor_form preserving order
        $grouped = [];
        foreach ($allRows as $row) {
            $nomor = trim($row['nomor_form'] ?? '');
            if ($nomor === '') {
        ...
        foreach ($grouped as $nomor => $formRows) {
            ...
            $penggunaNik = trim($penggunaNikRaw = $firstRow['pengguna_nik'] ?? '');
            $assetNo = trim($assetNoRaw = $firstRow['asset_no_asset'] ?? '');
            $siteLocation = trim($siteLocationRaw = $firstRow['site_location'] ?? '');
            ...
            $this->validForms[] = [
                ...
                'data' => [
                    ...
                    'kondisi' => strtolower(trim($firstRow['kondisi'] ?? '')) ?: null,
                    'kondisi_keterangan' => trim($firstRow['kondisi_keterangan'] ?? '') ?: null,
                    'notes' => trim($firstRow['notes'] ?? '') ?: null,
                    'tindakan_solution' => trim($firstRow['tindakan_solution'] ?? '') ?: null,
                    'status' => $status,
                ],
                'items' => $items,
            ];
    ...
    public function confirmImport()
    {
        ...
        try {
            foreach ($this->validForms as $validForm) {
                $form = FormPemeriksaan::create($validForm['data']);
        ...
        $message = "Import selesai: {$importedCount} form pemeriksaan berhasil diimpor.";

        ActivityLogger::log('import', "Mengimpor {$importedCount} form pemeriksaan");
    ...
    {
        return view('livewire.admin.pemeriksaan.import-csv');
    }
}
