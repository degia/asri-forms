        ...
        <div class="glass-card p-6 space-y-4">
            <div class="flex items-center gap-3 mb-2">
                ...
                <div>
                    ...
                    <p class="text-xs text-muted">{{ __('Format') }}: name, nik, site, directorate, divisi, departement, sub_departement, position, no_telepon, email, status, date_resign</p>
                </div>
            </div>
            ...
            <div class="relative border-2 border-dashed rounded-lg p-8 text-center transition-colors duration-200"
                ...
                <p class="text-xs text-muted">{{ __('Maksimal 10MB (.csv)') }}</p>
                    ...
                    <table class="w-full text-xs">
                        <thead>
                            <tr class="border-b" style="border-color: var(--color-border);">
                                ...
                                <th class="px-3 py-2 text-left text-muted font-medium whitespace-nowrap">Directorate</th>
                                <th class="px-3 py-2 text-left text-muted font-medium whitespace-nowrap">Divisi</th>
                                <th class="px-3 py-2 text-left text-muted font-medium whitespace-nowrap">Departement</th>
                                <th class="px-3 py-2 text-left text-muted font-medium whitespace-nowrap">Sub Departement</th>
                                <th class="px-3 py-2 text-left text-muted font-medium whitespace-nowrap">Position</th>
                        ...
                        <tbody class="divide-y" style="border-color: var(--color-border);">
                            @foreach($preview as $i => $row)
                                <tr>
                                    ...
                                    <td class="px-3 py-2 text-primary font-mono font-medium whitespace-nowrap">{{ $row['nik'] ?? '-' }}</td>
                                    <td class="px-3 py-2 text-secondary whitespace-nowrap">{{ $row['site'] ?? '-' }}</td>
                                    <td class="px-3 py-2 text-secondary whitespace-nowrap">{{ $row['directorate'] ?? '-' }}</td>
                                    <td class="px-3 py-2 text-secondary whitespace-nowrap">{{ $row['divisi'] ?? '-' }}</td>
                                    <td class="px-3 py-2 text-secondary whitespace-nowrap">{{ $row['departement'] ?? '-' }}</td>
                                    <td class="px-3 py-2 text-secondary whitespace-nowrap">{{ $row['sub_departement'] ?? '-' }}</td>
                                    <td class="px-3 py-2 text-secondary whitespace-nowrap">{{ $row['position'] ?? '-' }}</td>
                            ...
                            <table class="w-full text-xs">
                                <thead>
                                    <tr class="border-b" style="border-color: var(--color-border);">
                                        ...
                                        <th class="px-2 py-1.5 text-left text-muted font-medium whitespace-nowrap">NIK</th>
                                        <th class="px-2 py-1.5 text-left text-muted font-medium whitespace-nowrap">{{ __('Email') }}</th>
                                ...
                                <tbody class="divide-y" style="border-color: var(--color-border);">
                                    @foreach($importSuccess as $row)
                                        <tr>
                                            ...
                                            <td class="px-2 py-1.5 text-primary font-mono font-medium whitespace-nowrap">{{ $row['data']['nik'] }}</td>
                                            <td class="px-2 py-1.5 text-secondary whitespace-nowrap">{{ $row['data']['email'] }}</td>
                                            <td class="px-2 py-1.5 text-secondary whitespace-nowrap">{{ $row['data']['status'] }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
    ...
    @if($showCancelModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" style="background: rgba(0,0,0,0.5); backdrop-filter: blur(4px);"
            ...
            <div class="glass-card p-6 w-full max-w-md space-y-4" @click.away="$wire.dismissCancelImport()">
                ...
                @if($imported)
                    <p class="text-sm text-muted">{{ __('Yakin ingin membatalkan import? Data employee yang baru ditambahkan akan dihapus, dan employee yang diperbarui akan dikembalikan ke data sebelumnya.') }}</p>
                @else
    ...
    @if($showConfirmModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" style="background: rgba(0,0,0,0.5); backdrop-filter: blur(4px);"
            ...
            <div class="glass-card p-6 w-full max-w-md space-y-4" @click.away="$wire.dismissConfirmImport()">
                ...
                <p class="text-sm text-muted">{{ __('Yakin ingin mengirim') }} <span class="font-semibold text-primary">{{ $successCount }} {{ __('data employee') }}</span> {{ __('ke database? Setelah dikirim, Anda akan diarahkan ke halaman Employees.') }}</p>
                <div class="flex gap-2">
