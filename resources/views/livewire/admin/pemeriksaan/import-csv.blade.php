    ...
    @if(!$imported)
        <div class="glass-card p-6 space-y-4">
            <div class="flex items-center gap-3 mb-2">
                ...
                <div>
                    ...
                    <p class="text-xs text-muted">{{ __('Format multi-baris: baris pertama = header + item pertama, baris selanjutnya = nomor_form + item tambahan') }}</p>
                </div>
            </div>
        ...
        @if(!empty($preview) && empty($importErrors))
            <div class="glass-card p-6 space-y-4">
                <div class="flex items-center justify-between">
                    <h3 class="font-semibold text-primary">{{ __('Preview') }} ({{ $totalForms }} {{ __('form') }}, {{ $totalRows }} {{ __('baris') }})</h3>
                    ...
                    <table class="w-full text-xs">
                        <thead>
                            <tr class="border-b" style="border-color: var(--color-border);">
                                ...
                                <th class="px-3 py-2 text-left text-muted font-medium whitespace-nowrap">{{ __('No. Form') }}</th>
                                <th class="px-3 py-2 text-left text-muted font-medium whitespace-nowrap">{{ __('NIK Pengguna') }}</th>
                                ...
                                <th class="px-3 py-2 text-left text-muted font-medium whitespace-nowrap">{{ __('Lokasi') }}</th>
                                <th class="px-3 py-2 text-left text-muted font-medium whitespace-nowrap">{{ __('Kondisi') }}</th>
                                <th class="px-3 py-2 text-left text-muted font-medium whitespace-nowrap">{{ __('Status') }}</th>
                                <th class="px-3 py-2 text-left text-muted font-medium whitespace-nowrap">{{ __('Item') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y" style="border-color: var(--color-border);">
                            @foreach($preview as $i => $row)
                                <tr>
                                    ...
                                    <td class="px-3 py-2 text-primary font-mono font-medium whitespace-nowrap">{{ $row['nomor_form'] ?? '-' }}</td>
                                    <td class="px-3 py-2 text-secondary whitespace-nowrap">{{ $row['pengguna_nik'] ?? '-' }}</td>
                                    <td class="px-3 py-2 text-secondary font-mono whitespace-nowrap">{{ $row['asset_no_asset'] ?? '-' }}</td>
                                    <td class="px-3 py-2 text-secondary whitespace-nowrap">{{ $row['site_location'] ?? '-' }}</td>
                                    <td class="px-3 py-2 text-secondary whitespace-nowrap">{{ $row['location_detail'] ?? '-' }}</td>
                                    <td class="px-3 py-2 text-secondary whitespace-nowrap">{{ $row['kondisi'] ?? '-' }}</td>
                                    <td class="px-3 py-2 text-secondary whitespace-nowrap">{{ $row['status'] ?? '-' }}</td>
                                    <td class="px-3 py-2 text-secondary whitespace-nowrap">{{ $row['item_category'] ?? '' }} {{ $row['item_name'] ?? '' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
        ...
        <div class="glass-card p-6 space-y-4" style="border-color: rgba(245, 158, 11, 0.4);">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <h3 class="font-semibold text-primary">{{ __('Data Terbaca') }}: {{ $successCount }} {{ __('Form Berhasil') }}, {{ $errorCount }} {{ __('Form Gagal') }}</h3>
            ...
            @if($resultTab === 'berhasil')
                @if(count($importSuccess) > 0)
                    <div class="text-left mt-2 p-3 rounded-lg" style="background: var(--color-glass-bg); border: 1px solid var(--color-border);">
                        <p class="text-xs font-semibold text-emerald-400 mb-2">{{ __('Detail Data Berhasil') }} ({{ count($importSuccess) }} {{ __('form') }}):</p>
                        <div class="overflow-x-auto max-h-60 overflow-y-auto">
                            <table class="w-full text-xs">
                                <thead>
                                    <tr class="border-b" style="border-color: var(--color-border);">
                                        <th class="px-2 py-1.5 text-left text-muted font-medium whitespace-nowrap">{{ __('No. Form') }}</th>
                                        <th class="px-2 py-1.5 text-left text-muted font-medium whitespace-nowrap">{{ __('Pengguna') }}</th>
                                        <th class="px-2 py-1.5 text-left text-muted font-medium whitespace-nowrap">{{ __('No. Asset') }}</th>
                                        <th class="px-2 py-1.5 text-left text-muted font-medium whitespace-nowrap">{{ __('Items') }}</th>
                                        <th class="px-2 py-1.5 text-left text-muted font-medium whitespace-nowrap">{{ __('Status') }}</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y" style="border-color: var(--color-border);">
                                    @foreach($importSuccess as $row)
                                        <tr>
                                            <td class="px-2 py-1.5 text-primary font-mono font-medium whitespace-nowrap">{{ $row['nomor_form'] }}</td>
                                            <td class="px-2 py-1.5 text-secondary whitespace-nowrap">{{ $row['pengguna'] }}</td>
                                            <td class="px-2 py-1.5 text-secondary font-mono whitespace-nowrap">{{ $row['asset_no'] }}</td>
                                            <td class="px-2 py-1.5 text-secondary whitespace-nowrap">{{ $row['items_count'] }}</td>
                                            <td class="px-2 py-1.5 text-secondary whitespace-nowrap">{{ ucfirst($row['status']) }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
            ...
            @else
                @if(!empty($importErrors))
                    <div class="text-left mt-2 p-3 rounded-lg" style="background: var(--color-glass-bg); border: 1px solid var(--color-border);">
                        <p class="text-xs font-semibold text-red-400 mb-2">{{ __('Detail Error') }} ({{ $errorCount }} {{ __('form gagal') }}):</p>
    ...
    @if($showConfirmModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" style="background: rgba(0,0,0,0.5); backdrop-filter: blur(4px);"
            ...
            <div class="glass-card p-6 w-full max-w-md space-y-4" @click.away="$wire.dismissConfirmImport()">
                ...
                <p class="text-sm text-muted">{{ __('Yakin ingin mengirim') }} <span class="font-semibold text-primary">{{ $successCount }} {{ __('form pemeriksaan') }}</span> {{ __('ke database? Setelah dikirim, Anda akan diarahkan ke halaman Pemeriksaan.') }}</p>
                <div class="flex gap-2">
