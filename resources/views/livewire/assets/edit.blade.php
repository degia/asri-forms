<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-primary">{{ __('Edit Asset') }}</h1>
            <p class="text-sm text-muted mt-1">{{ $assetModel?->nama_perangkat }}</p>
        </div>
        <a href="{{ route('assets.show', $this->assetModel->id) }}" wire:navigate
            class="text-sm text-secondary hover:underline">
            &larr; {{ __('Kembali ke Detail') }}
        </a>
    </div>
    ...
    <div class="glass-card p-6 space-y-5"
        ...
        x-on:asset-updated.window="errors = {}; window.location = '{{ route('assets.show', $this->assetModel->id) }}'">
        ...
        <div>
            ...
            <div class="mt-2 flex items-end gap-3">
                ...
                @elseif ($this->assetModel?->foto_url)
                    <div>
                        <img src="{{ $this->assetModel->foto_url }}" alt="{{ $namaPerangkat }}"
                            class="h-32 w-32 rounded-lg object-cover border"
                            style="border-color: var(--color-border);" />
                        <button type="button" wire:click="removeFoto" wire:confirm="{{ __('Hapus foto ini?') }}"
                            class="mt-1 text-xs text-red-400 hover:underline">
                            {{ __('Hapus Foto') }}
                        </button>
                    </div>
                @endif
